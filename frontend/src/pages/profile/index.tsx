...
const Profile = ({ userProfile }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
...
export const getServerSideProps: GetServerSideProps<{
    ...
    if(!session?.accessToken){
        ...
        const res = await fetch(`http://${process.env.AUTH_URL}/api/v1/users/currentUser`, {
            method: 'GET',
            headers: { 
                'Content-Type': 'application/json',
                'Cookie': `session=${session.accessToken}`
            },
        })
